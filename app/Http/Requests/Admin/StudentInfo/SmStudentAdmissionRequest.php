...
class SmStudentAdmissionRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $rules= [
            ...
            'admission_number' => [Rule::unique('sm_students', 'registration_no')->ignore(optional($student)->id)->where('church_id', $church_id), Rule::requiredIf(function () use ($field) {
        ...
        if ($user_role_id !=2) {
            ...
            }), Rule::unique('sm_students', 'roll_no')->ignore(optional($student)->id)->where('church_id', $church_id)->where('church_year_id', $church_year_id)->whereIn('age_group_id', $age_group_ids)->whereIn('mgender_id', $mgender_ids)],
            ];
        }
