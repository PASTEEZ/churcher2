...
class SearchStudentController extends Controller
{
    ...
    public function viewTeacherRoutine()
    {
        try {
            ...
            $class_times = SmClassTime::where('church_id',Auth::user()->church_id)->get();
            ...
            $teachers = SmStaff::select('id', 'full_name')->where('active_status', 1)->where('church_id',Auth::user()->church_id)->get();
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                foreach ($weekenD as $row) {
                    $data[$row->name] = DB::table('sm_class_routine_updates')
                        ...
                        ])->where('sm_sections.church_id',Auth::user()->church_id)->get();
                }
    ...
    public function homeworkList2(Request $request, $id)
    {
        try {
            ...
            $homeworkLists = SmHomework::where('sm_homeworks.created_by', '=', $teacher_id)
                ...
                ->where('sm_homeworks.church_id',Auth::user()->church_id)->get();
    ...
    public function uploadContent(Request $request)
    {
        ...
        try {
            ...
            if ($request->input('available_for') == 'student') {
                if (!empty($request->input('all_classes'))) {
                    $students = SmStudent::select('id')->where('church_id',Auth::user()->church_id)->get();
                    ...
                    $students = SmStudent::select('id')->where('age_group_id', $request->input('class'))->where('mgender_id', $request->input('section'))->where('church_id',Auth::user()->church_id)->get();
                    foreach ($students as $student) {
                        $notification = new SmNotification;
                        $notification->user_id = $student->user_id;
                        $notification->role_id = 2;
