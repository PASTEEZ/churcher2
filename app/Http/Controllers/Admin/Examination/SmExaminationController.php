...
class SmExaminationController extends Controller
{
    ...
    public function previousClassResultsView($registration_no, Request $request)
    {
        ...
        try {
            ...
            $promotes = SmStudentPromotion::where('registration_number', '=', $registration_no)
                ...
                // ->select('registration_number', 'member_id', 'previous_age_group_id', 'age_group_name', 'previous_mgender_id', 'mgender_name', 'year', 'previous_session_id')
                ->get();
            if ($promotes->count() < 1) {
            ...
            $studentDetails = SmStudentPromotion::where('registration_number', '=', $registration_no)
                ->join('sm_academic_years', 'sm_academic_years.id', '=', 'sm_student_promotions.previous_session_id')
                ->join('sm_classes', 'sm_classes.id', '=', 'sm_student_promotions.previous_age_group_id')
                ->join('sm_students', 'sm_students.id', '=', 'sm_student_promotions.member_id')
                ->join('sm_sections', 'sm_sections.id', '=', 'sm_student_promotions.previous_mgender_id')
            ...
            if ($promotes->count() > 0) {
                ...
                return view('backEnd.reports.previousClassResults', compact('promotes', 'studentDetails', 'generalSetting', 'current_class', 'current_section', 'current_session', 'registration_number'));
            } else {
    ...
    public function previousStudentRecord(AdmissionNumberGetStudentRequest $request)
    {
        ...
        $studentInfo = SmStudent::where('registration_no', $registration_number)->first();
        if(!$studentInfo){
    ...
    {
        try {
            $promotes = SmStudentPromotion::where('registration_number', '=', $request->registration_number)
                ->join('sm_academic_years', 'sm_academic_years.id', '=', 'sm_student_promotions.previous_session_id')
                ->join('sm_classes', 'sm_classes.id', '=', 'sm_student_promotions.previous_age_group_id')
                ->join('sm_students', 'sm_students.id', '=', 'sm_student_promotions.member_id')
                ->join('sm_sections', 'sm_sections.id', '=', 'sm_student_promotions.previous_mgender_id')
                // ->select('registration_number', 'member_id', 'previous_age_group_id', 'age_group_name', 'previous_mgender_id', 'mgender_name', 'year', 'previous_session_id')
                ->get();
            $studentDetails = SmStudentPromotion::where('registration_number', '=', $request->registration_number)
                ->join('sm_academic_years', 'sm_academic_years.id', '=', 'sm_student_promotions.previous_session_id')
                ->join('sm_classes', 'sm_classes.id', '=', 'sm_student_promotions.previous_age_group_id')
                ->join('sm_students', 'sm_students.id', '=', 'sm_student_promotions.member_id')
                ->join('sm_sections', 'sm_sections.id', '=', 'sm_student_promotions.previous_mgender_id')
    ...
    public function marksRegisterStore(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->member_ids as $member_id) {
                ...
                $member_id_no = ($request->student_rolls[$sid] == null) ? '' : $request->student_rolls[$sid];
            ...
            if (moduleStatusCheck('University')) {
                ...
                if ($request->method() == 'POST') {
                    ...
                    foreach ($eligible_students as $SingleStudent) {
                        ...
                        $full_name = $SingleStudent->full_name; //get name
                        $registration_no = $SingleStudent->registration_no; //get admission no
                        $member_id_no = $SingleStudent->member_id_no; //get admission no
                        $member_id = $SingleStudent->id; //get admission no
                        ...
                        $insert_results->member_name = $full_name;
                        $insert_results->registration_no = $registration_no;
                        $insert_results->member_id_no = $member_id_no;
