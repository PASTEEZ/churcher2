...
use App\Http\Requests\Admin\StudentInfo\SmMemberRegistrationRequest;
...
class SmMemberRegistrationController extends Controller
{
    use CustomFields;
    use FeesAssignTrait;
    ...
    public function index()
    {
        ...
        try {
            if (isSubscriptionEnabled() && auth()->user()->church_id != 1) {
                ...
                $active_student = SmJYMember::where('church_id', Auth::user()->church_id)->where('active_status', 1)->count();

                if (\Modules\Saas\Entities\SmPackagePlan::student_limit() <= $active_student) {

                    Toastr::error('Your member limit has been crossed.', 'Failed');
            ...
            $data = $this->loadData();
            $data['max_admission_id'] = SmJYMember::where('church_id', Auth::user()->church_id)->max('id');
            $data['max_roll_id'] = SmJYMember::where('church_id', Auth::user()->church_id)->max('member_id_no');

        
            return view('backEnd.studentInformation.jy_registration', $data);
    ...
    public function store(SmMemberRegistrationRequest $request)
    {
        ...
        try {
            ...
            $user_stu->username = $request->phone_number ?: ($request->email_address ?: $request->registration_number);
            ...
            if ($request->roll_number) {
                $student->member_id_no = $request->registration_number;
            }
    ...
    public function edit(Request $request, $id)
    {
        try {
            $data = $this->loadData();
            $data['student'] = SmJYMember::with('sections')->select('sm_jymembers.*')->find($id);
            $data['siblings'] = SmJYMember::where('parent_id', $data['student']->parent_id)->whereNotNull('parent_id')->where('id', '!=', $id)->get();
    ...
    public function updateStudentInfo($request, $studentRecord = null)
    {
        ...
        $parentInfo = ($request->fathers_name || $request->fathers_phone || $request->mothers_name || $request->mothers_phone || $request->guardians_email || $request->guardians_phone )  ? true : false;
        $student_detail = SmJYMember::find($request->id);
        ...
        $destination = 'public/uploads/student/document/';
        $student_file_destination = 'public/uploads/student/';
        $student = SmJYMember::find($request->id);
        if ($request->relation == 'Father') {
        ...
        try {
            ...
            if ($studentRecord && generalSetting()->multiple_roll == 0 && $request->roll_number) {
                $studentRecord->update(['member_id_no'=>$request->registration_number]);
            }
            DB::commit();
    ...
    public function update(SmMemberRegistrationRequest $request)
    {
        ...
        try {
            ...
            if (generalSetting()->multiple_roll == 0 && $request->roll_number && $studentRecord) {
                ...
                ->where('member_id_no', $request->roll_number)
    ...
    public function view(Request $request, $id, $type = null)
    {
        try {
            $next_labels = null;
            $student_detail = SmJYMember::withoutGlobalScope(StatusAcademicSchoolScope::class)->find($id);
            ...
            if(moduleStatusCheck('University')){
                ...
                $studentDetails = SmJYMember::find($member_id);
    ...
    public function studentDetails(Request $request)
    {
        try {
            ...
            $students = SmJYMember::where('church_year_id', getAcademicId())
                ->where('church_id', Auth::user()->church_id)
                ->get();
        ...
        try {
            ...
            if (generalSetting()->multiple_roll == 1 && $request->roll_number) {
                $exitRoll = StudentRecord::where('age_group_id', $request->class)
                    ->where('mgender_id', $request->section)
                    ->where('member_id_no', $request->roll_number)->where('church_id', auth()->user()->church_id)->first();
    ...
    public function insertStudentRecord($request, $pre_record = null)
    {
        ...
        if (generalSetting()->multiple_roll == 0 && $request->roll_number) {
            ...
            ->when(moduleStatusCheck('University'), function ($query) {
                ...
                $query->where('church_year_id', getAcademicId());
                    ...
                    'member_id_no' => $request->roll_number,
                ]);
        ...
        if ($request->roll_number) {
            $studentRecord->member_id_no = $request->registration_number;
        }
        ...
        $student = SmJYMember::where('church_id', auth()->user()->church_id)->find($request->member_id);
        if($student){
            $user = $student->user;
            foreach($groups as $group){
    ...
    public function assignClass($id)
    {
        ...
        $data['student_detail'] = SmJYMember::where('id', $id)->first();
    ...
    public function recordEdit($member_id, $record_id)
    {
        ...
        $data['student_detail'] = SmJYMember::where('id', $member_id)->first();
        $data['classes'] = SmClass::get(['id', 'age_group_name']);
            ...
            try {
                ...
                if (!empty($data)) {
                    foreach ($data as $key => $value) {
                                ...
                                if ($user) {
                                    if ($user->role_id == 2) {
                                        ...
                                        if (!$studentRecord) {
                                            $this->insertStudentRecord($request->merge([
                                                'member_id' => $user->student->id,
                                                'roll_number'=>$request->registration_number
                                            ]));
                        ...
                        if ($value->email != "") {
                            $chk = DB::table('sm_jymembers')->where('email', $value->email)->where('church_id', Auth::user()->church_id)->count();
                            if ($chk >= 1) {
                                DB::rollback();
                                StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        $this->insertStudentRecord($request->merge([
                                            'member_id' => $student->id,
                                            'is_default' => 1,
                                            'roll_number'=>$value->registration_number
                                        ]));
