...
class SmClassRoutineNewController extends Controller
{
    ...
    public function addNewClassRoutine($class_time_id, $day, $age_group_id, $mgender_id)
    {

        try {
            $assinged_subjects = SmClassRoutineUpdate::select('subject_id')->where('age_group_id', $age_group_id)->where('mgender_id', $mgender_id)->where('day', $day)
            ...
            $stds = SmStudent::where('age_group_id', $age_group_id)->where('mgender_id', $mgender_id)->where('church_year_id', getAcademicId())->where('church_id',Auth::user()->church_id)->count();
            $rooms = SmClassRoom::where('active_status', 1)->where('capacity','>=',$stds)->where('church_id',Auth::user()->church_id)->get();
            $subjects = SmAssignSubject::where('age_group_id', $age_group_id)->where('mgender_id', $mgender_id)->where('church_id',Auth::user()->church_id)->get();

            return view('backEnd.academics.add_new_class_routine_form', compact('rooms', 'subjects', 'day', 'class_time_id', 'age_group_id', 'mgender_id', 'assinged_subject', 'assinged_room'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function addNewClassRoutineEdit($class_time_id, $day, $age_group_id, $mgender_id, $subject_id, $room_id, $assigned_id, $teacher_id)
    {
        ...
        try {
            ...
            $rooms = SmClassRoom::where('active_status', 1)->where('church_id',Auth::user()->church_id)->get();
            $teacher_detail = SmStaff::select('id', 'full_name')->where('id', $teacher_id)->first();

            $subjects = SmAssignSubject::where('age_group_id', $age_group_id)->where('mgender_id', $mgender_id)->where('church_id',Auth::user()->church_id)->get();
            return view('backEnd.academics.add_new_class_routine_form', compact('rooms', 'subjects', 'day', 'class_time_id', 'age_group_id', 'mgender_id', 'assinged_subject', 'assinged_room', 'subject_id', 'room_id', 'assigned_id', 'teacher_detail'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function addNewClassRoutineStore(Request $request)
    {
        try {
                ...
                $days=$request->day_ids;
                    if($request->day_ids){
                     foreach ($days as $day) {
                         ...
                         $check = SmClassRoutineUpdate::where('age_group_id', $request->age_group_id)
                                                     ->where('mgender_id', $request->mgender_id)->where('subject_id', $request->subject)
            ...
            //return view('backEnd.academics.class_routine_new', compact('classes', 'class_times', 'age_group_id', 'mgender_id'));
    ...
    public function getClassTeacherAjax(Request $request)
    {
        ...
        try {
            $subject_teacher = SmAssignSubject::select('teacher_id')->where('age_group_id', $request->age_group_id)->where('mgender_id', $request->mgender_id)->where('subject_id', $request->subject)->first();
            $teacher_detail = '';
            $i = 0;
    ...
    public function classRoutineReportSearch(Request $request)
    {
        ...
        try {
            ...
            $sm_routine_updates = $classes->where('id',$request->class)->first()->routineUpdates->where('mgender_id',$mgender_id);
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
                $data['classes'] = $classes->toArray();
                $data['class_times'] = $class_times->toArray();
            ...
            return view('backEnd.reports.class_routine_report', compact('classes', 'class_times', 'age_group_id', 'mgender_id','sm_routine_updates', 'sm_weekends'));
        } catch (\Exception $e) {
