            ...
            if (!empty($bulk_staffs)) {
                foreach ($bulk_staffs as $key => $singleStaff) {
                    ...
                    if ($this->checkExitUser($singleStaff->mobile, $singleStaff->email) || !$role_id) {
                        $exit_user[] = $singleStaff->mobile ?? $singleStaff->email;
                        continue;
                    }
                    ...
                    if ($user) {
                        ...
                        $staff->day_born = $singleStaff->day_born;
                        $staff->employer_name = $singleStaff->employer_name;
                ...
                DB::commit();
                if(!empty($exit_user)) {
                    Toastr::warning(implode(',', $exit_user).' !already Exit', 'Warning');
                }
